...
function default_new(){
    ...
    if($_POST) {
        $lans = $_POST['answer'];
        unset($_POST['answer']);
        $array_info = $_POST['info'];
        unset($_POST['info']);
        $_POST['slug']=$core->toSlug($_POST['title']);
        $slug = $_POST['slug'];
        if($_POST['is_push']==1) $_POST['push_date']=time();
        ...
        if($clsClassTable->insertOne($_POST)) {
            ...
            foreach($lans as $key=>$ans) {
                if($ans) {
                    $image = '';
                    if($_FILES['image_'.$key]['name']) {
                        $image = $core->ftpUpload('image_'.$key, 'files', $slug.$key.rand(0,9), time(),true);
                    }
                    
                    $ans_id = $clsAvote->insertOne(array("title"=>$ans,"info"=>$array_info[$key],"image"=>$image,"vote_id"=>$insert_id));
                
                }
            }
            
            header('location: ?mod='.$mod.'&act=edit&id='.$insert_id.'&mes=insertSuccess');
        }
        else {
...
function default_edit(){
    ...
    if($_POST) {
        $_POST['slug']=$core->toSlug($_POST['title']);
        if($_POST['is_push']==1 && !$oneItem['is_push']) $_POST['push_date'] = time();
        ...
        foreach($_POST['answer'] as $key=>$ans) {
            if($ans && $key=='new') {
                foreach($ans as $k=>$a) {
                    ...
                    if($_FILES['image_'.$key]['name']) {
                        $image = $core->ftpUpload('image_'.$k, 'files', $slug.$k.rand(0,9), time(),true);
                    }
                    
                    if($a) $ans_id = $clsAvote->insertOne(array("title"=>$a,"image"=>$image,"info"=>$_POST['info'][$k],"vote_id"=>$_GET['id']));
                ...
                foreach($ans as $k=>$a) {
                    ...
                    $image = '';
                    if($_FILES['image_'.$k]['name']) {
                        $image = $core->ftpUpload('image_'.$k, 'files', $slug.$k.rand(0,9), time(),true);
                    }
                    if($a && $image) $ans_id = $clsAvote->updateOne($k,array("title"=>$a,"image"=>$image,"info"=>$_POST['info'][$k]));
                    else $ans_id = $clsAvote->updateOne($k,array("title"=>$a,"info"=>$_POST['info'][$k]));
                }
            }
        }

        unset($_POST['answer']);unset($_POST['info']);
