...
class Moxiecode_LocalFileImpl extends Moxiecode_BaseFile {
	...
	function &listFilesFiltered(&$filter) {
		...
		if ($fHnd = opendir($this->_manager->toOSPath($dir))) {
            ...
            $sortedFiles = array(); foreach($fileArray as $file) { $key = filemtime($dir . "/" . $file); while(isset($sortedFiles[$key])) $key++; $sortedFiles[$key] = $file; } ksort($sortedFiles); $fileArray = array_reverse(array_values($sortedFiles));
